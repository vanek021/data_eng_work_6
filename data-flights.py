...
file_name = "./data/flights/flights.csv"
...
utils.handle_file_size(dataset, file_name, "./data/flights/file_size_unopt_result.json")
...
utils.handle_file_size(optimized_dataset, file_name, "./data/flights/file_size_opt_result.json")
...
for chunk in pd.read_csv(file_name, usecols=lambda x: x in column_names, dtype=need_column, chunksize=100_000):
    ...
    chunk.to_csv("./data/flights/df_sub.csv", mode="a", header=has_header)
    has_header = False

dtype_json = need_column.copy()
for key in dtype_json.keys():
    dtype_json[key] = str(dtype_json[key])
utils.write_to_json("./data/flights/dtypes.json", dtype_json)