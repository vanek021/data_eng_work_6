...
file_name = "./data/automotive/automotive.csv.zip"

# 1. Загрузить набор данных из файла
dataset = utils.read_file(file_name, "zip")
...
utils.handle_file_size(dataset, file_name, "./data/automotive/file_size_unopt_result.json")
...
utils.handle_file_size(optimized_dataset, file_name, "./data/automotive/file_size_opt_result.json")
...
column_names = ['vin','stockNum','firstSeen','lastSeen','msrp','askPrice','mileage','isNew','color','interiorColor','brandName','modelName','dealerID']
...
for chunk in pd.read_csv(file_name, usecols=lambda x: x in column_names, dtype=need_column, chunksize=100_000):
    ...
    chunk.to_csv("./data/automotive/df_sub.csv", mode="a", header=has_header)
    has_header = False

dtype_json = need_column.copy()
for key in dtype_json.keys():
    dtype_json[key] = str(dtype_json[key])
utils.write_to_json("./data/automotive/dtypes.json", dtype_json)